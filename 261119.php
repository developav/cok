<!DOCTYPE html>
<html dir="ltr" lang="ru">
<head>

	<?php include "includes/head-meta.php"; ?>

	<title>Новости | ЦОК</title>

</head>

<body class="stretched">

	<!-- Document Wrapper
	============================================= -->
	<div id="wrapper" class="clearfix">

		<!-- Header
		============================================= -->
		<header id="header" class="full-header">

			<?php include "includes/primary-menu.php"; ?>

		</header><!-- #header end -->

		<!-- Page Title
		============================================= -->
		<section id="page-title">

			<div class="container clearfix">
				<!-- <h1>Новости</h1> -->
				<ol class="breadcrumb">
					<li><a href="index.php">Главная</a></li>
					<li><a href="index.php#cok_news">Новости</a></li>
				</ol>
			</div>

		</section><!-- #page-title end -->

		<!-- Content
		============================================= -->
		<section id="content">

			<div class="content-wrap">

				<div class="container clearfix">

					<!-- Post Content
					============================================= -->
					<div class="single-post nobottommargin">

						<!-- Single Post
						============================================= -->
						<div class="entry clearfix">

							<!-- Entry Title
							============================================= -->
							<div class="entry-title">
								<h3>Мы  расширяем границы сотрудничества.</h3>
							</div><!-- .entry-title end -->

							<!-- Entry Meta
							============================================= -->
							<ul class="entry-meta clearfix">
								<li><i class="icon-calendar3"></i> 26 Ноября 2019</li>
							</ul><!-- .entry-meta end -->

							<!-- Entry Image
							============================================= -->

							<!-- Entry Content
							============================================= -->
							<div class="entry-content notopmargin" style="text-align: justify;">

                                <!--<div class="image bottommargin-sm"><img src="images/news/full/95.jpg" style="display: block; margin: 0 auto;"></div>-->

                                <!--<div class="image bottommargin topmargin-sm col_one_third">
                                    <img src="images/news/full/105.png" alt="News Single">
                                </div>--><!-- .entry-image end -->

                                <div class="image bottommargin-sm">
                                    <img src="images/news/grid/105.png" style="display: block; margin: 0 auto;">
                                </div>

                                <p>20.11 2019 состоялась рабочая встреча с представителем Южного федерального университета Лысенко И.Е., заведующим кафедрой конструирования электронных средств Института нанотехнологий. </p>
                                <p>В ходе визита были рассмотрены перспективные направления сотрудничества. К таким направлениям можно отнести: обучение по инновационным направлениям наноидустрии, прохождение производственной
                                    практики, обмен группами для организации стажировок и создание экзаменационного центра на базе ЮФУ. На встрече присутствовали руководители и ведущие разработчики  структурных
                                    подразделений АО «НИИМЭ». По результатам встречи были  достигнуты важные соглашения, направленные на расширение и развития взамовыгодного партнерства.
                                    Мы заинтересованы в перспективных проектах и готовы открывать новые горизонты сотрудничества.</p>
                                <p></p>
                                <p></p>
                                <p></p>
                                <p></p>
                                <p></p>
                                <p></p>

                                <!--<p><a href="https://nark.ru/news/intervyu-a-n-leybovicha-rossiyskoy-gazete-kak-tran.php" target="_blank">Далее... </a></p>-->

							</div>

                            <div class="clear"></div>

                            <div class="image bottommargin topmargin-sm col_half">
                                <img src="images/news/full/105-1.png" alt="News Single">
                            </div>

                            <div class="image bottommargin topmargin-sm col_last">
                                <img src="images/news/full/105-2.png" alt="News Single">
                            </div>

						</div><!-- .entry end -->

					</div>

				</div>

			</div>

		</section><!-- #content end -->

		<!-- Footer
		============================================= -->
		<?php include "includes/footer.php"; ?>
		<!-- #footer end -->

	</div><!-- #wrapper end -->

	<!-- Go To Top
	============================================= -->
	<div id="gotoTop" class="icon-angle-up"></div>

	<!-- External JavaScripts
	============================================= -->
	<script type="text/javascript" src="js/jquery.js"></script>
	<script type="text/javascript" src="js/plugins.js"></script>

	<script src="js/menu-hilight.js"></script>

	<!-- Footer Scripts
	============================================= -->
	<script type="text/javascript" src="js/functions.js"></script>

	<script type="text/javascript">

	</script>

</body>
</html>